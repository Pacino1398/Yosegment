    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                frame_suffix = '' if dataset.mode == 'image' else f'_frame{frame:06d}'
                source_stem = f"{Path(path).stem}{frame_suffix}"
                for j, mask in enumerate(masks):
                    mask_img = (mask.cpu().numpy() * 255).astype(np.uint8)
                    save_name = f"{cls_ids[j]}_{source_stem}_{j}.png"
                    cv2.imwrite(os.path.join(mask_save_dir, save_name), mask_img)
                # ==============================================================
